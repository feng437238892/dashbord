      ...
      >
        <div class="dialog-content">
          ...
          <div v-if="showType === enumShowType.wxHotel">
            ...
            <!--</div>-->
            <div class="item-form">
              <el-select class="el-right" v-model="cityValue" placeholder="城市服务">
                <el-option
          ...
          <div v-if="showType === enumShowType.WxHotelRegister">
            <div style="font-size: 14px;font-weight: 400;color: #6d6e6e;margin-left: 35px;line-height: 2em">
              <p style="margin-top: 30px"><span style="margin-right: 20px">微信酒店ID:</span>
                <label>{{RegistersWxHotel ? RegistersWxHotel : '系统异常'}}</label></p>
              <p style="margin-top: 30px"><span style="margin-right: 20px">说明：</span><label>888888</label></p>
              <div>
                <div v-if="RegistersWxHotel">
...
//        wxhotelCityser: state => state.enterprise.wxhotelCityser,
      ...
      renderList(){
        return this.serviceCityserList;
      },
//      renderRegistersList(){
...
//      },
      ...
      validatewxHotelReg() {
//        return tool.isNotBlank(this.wxHotelId);
        this.cityValue && this.cityValue.length > 0 ? true : false;
      },
      validateminiApp() {
        if (!this.provider) {
...
//          this.faceEqu = configData.support_face_in === true ? "true" : "false"
    ...
    methods: {
      ...
      getCityservices(){
        this.getCityservice({
          onsuccess: body => (this.serviceCityserList = [...body.data])
        })
      },
      WxhotelRegisters(){
        this.WxhotelRegister({
          hotel_id: this.$route.params.hotelid,
          route_code: this.cityValue,
//          onsuccess: body => (this.wxHotelRegistersList = [...body.data])
          onsuccess: body => (this.RegistersWxHotel = body.data.wx_hotel_id)
        })
        this.showtoast({
          text: '注册成功',
          type: 'success'
...
//      删除微信生态酒店配置
      deleteWxHotels(){
        this.deleteWxHotel({
          hotel_id: this.$route.params.hotelid,
          wx_hotel_id: this.RegistersWxHotel,
